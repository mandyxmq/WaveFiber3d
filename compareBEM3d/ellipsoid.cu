...
int main(int argc, char **argv)
{
  ...
  float radius1 = 10 * 1e-6;
  float radius2 = 10 * 1e-6;
  float period = 10 * 1e-6;
  ...
  float beta = 6.0;
  float samplingenlargeratio = 3;
  float xrange = radius1;
  float yrange = radius2;
  float zrange = period * 0.5;
  ...
  anglenumvec[0] = obnum;
  int maxnum = obnum;
  std::cout << "level " << 0 << " thetaonum " << thetaonumvec[0] << " phionum " << phionumvec[0] << " anglenumvec[0] " << anglenumvec[0] << std::endl;
  ...
  for (int i = 1; i < max_depth + 1; ++i)
  {
    ...
    std::cout << "level " << i << " thetaonum " << thetaonumvec[i] << " phionum " << phionumvec[i] << " anglenumvec[i] " << anglenumvec[i] << " maxnum " << maxnum << std::endl;
  }
  ...
  for (int i = 0; i < max_depth; ++i)
  {
    ...
    std::cout << "plan_theta_ inembed[0] " << inembed[0] << " onembed[0] " << onembed[0] << " num[0] " << num[0] << " istride " << phionumvec[i] << " idist " << 1 << " ostride " << 1 << " odist " << thetaonumvec[i + 1] << " batch " << batch << std::endl;
  ...
  for (int lambdaindex = 0; lambdaindex < 1; ++lambdaindex)
  {
    ...
    for (int polarindex = 0; polarindex < 2; ++polarindex)
    {
      ...
      for (int i = max_depth; i >= 0; i--)
      {
        ...
        std::cout << "level " << i << " nodeindexstart " << nodeindexstart << " nodeindexend " << nodeindexend << " curthetaonum " << curthetaonum << " curphionum " << curphionum << std::endl;
              ...
              for (int childindex = 0; childindex < 8; ++childindex)
              {
                ...
                for (int searchindex = 0; searchindex < activechildlist.size(); ++searchindex)
                {
                  ...
                  if (activechildlist[searchindex] == truechildindex)
                  {
                    ...
                    sumchildren<<<numberblock, NUM_THREADS_PER_BLOCK>>>(thrust::raw_pointer_cast(tmp1_ex.data()),
        ...
        if (i == 0)
        {
          ...
          std::string filename = outputdir + "/ellipsoid_lambda" + std::to_string(lambdaindex) + "_T" + mode + "_depth" + std::to_string(max_depth) + "_kappa0.3.binary";
